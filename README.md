# 📖 Complete Seerah Timeline — Prophet Muhammad ﷺ

A beautifully designed and interactive website that presents the complete Seerah (biography) timeline of Prophet Muhammad ﷺ in a structured and easy-to-understand format.

## 🌟 Features

- 📅 Complete chronological Seerah timeline
- 🕌 Major Islamic historical events included
- 📖 Detailed descriptions for each event
- 🎨 Clean and modern responsive UI
- 🌙 Islamic-themed design
- ⚡ Fast and lightweight performance
- 📱 Mobile-friendly layout
- 🔍 Easy navigation through timeline sections

---

# 🌿 Asma-ul-Husna Website

An elegant website showcasing the 99 Names of Allah (Asma-ul-Husna) with meanings and descriptions in a clean and user-friendly interface.

## ✨ Features

- 📜 Complete 99 Names of Allah
- 🌍 Arabic names with English transliteration
- 📖 Meaning and explanation for each name
- 🎨 Beautiful Islamic-inspired design
- 📱 Fully responsive for all devices
- ⚡ Optimized and fast loading
- 🌙 Smooth user experience

---

# 🛠️ Tech Stack

- HTML5
- CSS3
- JavaScript
- Responsive Design
- Modern UI Components
